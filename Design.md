...
- Utilizes Ollama, a local Large Language Model service (running either OpenAI gpt-4o-mini or a local install of llama-3.2), to generate text summaries for each book. In a production setting, this application would not use the local llama-3.2 install, but would instead use either external LLM provider (e.g. OpenAI, Huggingface, Amazon Bedrock, etc.) or an internally-hosted version of the LLM, so it could be run on more appropriate hardware (and be scalable). https://ollama.com/, https://github.com/ollama/ollama-python
...
3. The search API retrieves book metadata from the OpenLibrary API using the provided title, then returns that to the UI for immediate rendering.
4. The UI requests from the backend all of the coverURLs in a batch call to the book API. Simultaneously, it starts retrieving summaries of the books.
...
5. As each new piece of information comes in (urls, summaries), the updated books are displayed in the frontend.

## Tradeoffs

- While it would be cleanest to get all the data for a book (including the summary) and send it back as a single JSON object, that will likely cause too much latency, since LLMs are relatively slow (as is the service to find the book covers). Thus, the UI will request the most straightfoward data immediately, and then improve it in subsequent calls. In a non-POC, it would make sense to use a websocket (e.g. via graphql) to send the data back as it becomes available rather than using a 1+n fetch model to get the more expensive data. Similarly, it would make sense to pre-fetch at least some of the data for the next page of results, assuming sufficient server capacity and click-throughs to the next page of results.

- This was originally developed using ollama and a locally-running LLM for generating the summaries. Because the local LLM had very limited capacity, the UI uses plimit to make sure there are never more than two outstanding calls to generate summaries. Limiting the concurrency decreases time to first summary while keeping the GPU maximally busy. If using an external service like OpenAI, this is irrelevant. An improvement would be (a) to fetch get the simultaneous request limit from the backend and use that to the limit the number of concurrent summary calls from the front-end or (b) to use a different back-end service (e.g. graphql) that could stream the results back as they are obtained. This would allow the backend to completely isolate the front-end from the details of how the LLMs are accessed.
...
4. Future: Add automated tests and develop additional features, such as user authentication, data streaming, and data encryption, as well as whatever the real application should be.
